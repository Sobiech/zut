    ...
    public partial class Form1 : Form {
        ...
        private void encodeMessage_Click(object sender, EventArgs e) {
            try {
                ...
                byte[] stegHash = shaM.ComputeHash(Encoding.UTF8.GetBytes(tbStegKey.Text));

                Encoder encoder = new Encoder(passHash, stegHash);
        ...
        private void decodeMessage_Click(object sender, EventArgs e) {
            try {
                ...
                Decoder decoder = new Decoder(passHash, stegHash);
        ...
        private string getPasswordAndValidateInputs(Boolean encode = false) {
            ...
            if (String.IsNullOrEmpty(tbStegKey.Text))
                throw new ArgumentException("Steganographic key is not set");
